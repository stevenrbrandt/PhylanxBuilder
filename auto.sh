...
do
    ...
    docker run --rm phylanx-test${TAG} cat test-out.txt > test-out-$bt.txt
    echo $EMAIL > email-body-$bt.html
    echo 'Phylanx Build Status' >> email-body-$bt.html
    rm -f email-body-$bt.txt
    touch email-body-$bt.txt
    set +e
    python3 parse.py 
    if [ $? = 0 ]
    then
      ...
      set -e
      echo 'Succeeded in building phylanx devenv.' >> email-body-$bt.txt
    else
      echo 'Failed to build phylanx devenv.' >> email-body-$bt.txt
    fi
    echo '' >> email-body-$bt.txt
    touch test-out-$bt.txt
    tail -20 test-out-$bt.txt >> email-body-$bt.txt
    mail -s "Auto Update Phylanx for $bt" ${EMAIL} < email-body-$bt.txt
done
